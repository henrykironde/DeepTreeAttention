...
def filter_data(path, config):
    ...
    dead_trees = shp[~shp.plantStatus.str.contains("LIVE")]
    dead_trees = dead_trees[dead_trees.siteID.isin(["OSBS","JERC","DSNY","TALL","LENO","DELA"])]
    dead_trees = dead_trees[dead_trees.plantStatus.str.contains("DEAD") & ~dead_trees.plantStatus.str.contains("LOST")]
    dead_trees["taxonID"] = "DEAD"
    
    shp = shp[shp.plantStatus.str.contains("LIVE")]
    shp = pd.concat([shp, dead_trees])
